...
public class FastdfsClientImpl implements FastdfsClient{
	...
	public String getUrl(String fileId) throws Exception{
		...
		try {
			...
			if(result.getCode()==0){
				String hostPort = getDownloadHostPort(result.getData());//TODO
				hostPort =":8080";
				url = "http://"+result.getData().split(":")[0]+hostPort+"/"+fastDfsFile.group+"/"+fastDfsFile.fileName;
			}
		} catch (Exception e) {
			logger.error(e.getMessage());
			throw e;
	...
	public String upload(byte[] fileArray, String fileName) throws Exception{
		String trackerAddr = getTrackerAddr();
		TrackerClient trackerClient = null;
		StorageClient storageClient = null;
		String storageAddr = null;
		...
		try {
			...
			if(result.getCode()==0){
				storageAddr = result.getData().getAddress();
				storageClient = storageClientPool.borrowObject(storageAddr);
				Result<String> result2 = storageClient.upload(fileArray, fileName, result.getData().getPathIndex());
				if(result2.getCode()==0){
					fileId = result2.getData();
				}
			}
