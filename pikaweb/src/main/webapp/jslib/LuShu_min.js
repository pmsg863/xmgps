...
(function() {
     ...
     * 动态设置路书位置
     */
    LuShu.prototype.step= function(step){
        var me = this;
        me.i = step;
        me._moveNext(step);
    }
     ...
     * 动态设置路书路径显示
     */
    LuShu.prototype.addPaths= function(paths){
    ...
    baidu.object.extend(LuShu.prototype, {
        ...
        _moveNext: function(index) {
            var me = this;
            //回调设置进度
            if (me._opts.mycallback!=null && me._opts.mycallback!="undefined"){
                var percent = me.i*100/me._path.length;
                me._opts.mycallback(percent);
            }
            if (index < this._path.length - 1) {
                var bound = this._map.getBounds();
                if ( !bound.containsPoint(me._path[index] ) )
                    this._map.panTo( me._path[index],new BMap.Point(0, 0)  );
