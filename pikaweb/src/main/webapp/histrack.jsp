...
<%--
        <div class="mainlist_input">
            <input style="line-height:35px;width: 180px;" class="mainlist_input_item_register" id="carno"
...
function trackreplay() {
    var paths = new Array();
    var i = 0;
    $.each(routetrack, function (index, track) {
        paths = paths.concat(track.trackpath)
    });

    function resetMkPoint(i) {
        carMk.setPosition(paths[i]);
        map.panTo(paths[i]);
        /* 设置进度*/
        var step = i*100/paths.length;
        $("#pos1")[0].innerHTML = Math.round( step );
        $("#slider").slider( "option", "value", Math.round( step ) );
...
function getbaidutrack(index, firsttrack, secondtrack) {
    var driving = new BMap.DrivingRoute(map);    //驾车实例
    var startpoint = WGS2BD(new BMap.Point(firsttrack.longtitude / 1000000, firsttrack.latitude / 1000000), new BMap.Point(0, 0));    //起点
    var endpoint = WGS2BD(new BMap.Point(secondtrack.longtitude / 1000000, secondtrack.latitude / 1000000), new BMap.Point(0, 0));    //终点
    driving.search(startpoint, endpoint);    //显示一条建议行驶线路

    driving.setSearchCompleteCallback(function () {
        var route = driving.getResults().getPlan(0).getRoute(0);    //获取策略 0

        var myTracks = new Object;
        myTracks.steps = route.getNumSteps();

        var stepinfo = new Array();
        for (var i = 0; i < myTracks.steps; i++) {
            var step = route.getStep(i);
            var carpos = new Object;
            carpos.id = firsttrack.id;
            carpos.carnumber = firsttrack.carnumber;
            carpos.location = 0;
        ...
        myTracks.stepsinfo = stepinfo;
        myTracks.trackpath = route.getPath();

        var polyline = new BMap.Polyline(myTracks.trackpath, {
            strokeColor: "blue",
            strokeWeight: 3,
            strokeOpacity: 0.5,
        ...
        success: function (data) {
            ...
            $.each(cartrack, function (index, track) {
                if (typeof(lasttrack) == "undefined") {
                    ...
                    carMk = new BMap.Marker(firstpoint, {icon: myIcon});
                    ...
                    /*getbaidutrack(index - 1, lasttrack, track);*/
                }
            })
            gettrack();
        }
